...
from typing import cast, List, Optional  # 新增 List
...
from ui.component.rule_editor_form import RuleEditorForm  # 新增
import db_manager
from app_models import TradingPair, AlertRule  # 新增 AlertRule
from alert_system.alert_processor import AlertProcessor  # 新增

logger = logging.getLogger(__name__)

# --- 全局变量 ---
pcm_instance: PublicChannelManager | None = None
alert_processor_instance: AlertProcessor | None = None  # 新增 AlertProcessor 实例
trading_pair_cards: dict[str, TradingPairCard] = {}
cards_container: ui.grid | None = None


def price_update_handler_factory(pair_id: int, inst_id: str):  # 修改：增加 pair_id
    """为指定instId创建价格更新回调处理函数。"""

    async def handler(price: str):  # 改为 async def 以便调用异步的 process_price_data (如果需要)
        # 更新UI卡片价格
        if inst_id in trading_pair_cards and trading_pair_cards[inst_id].is_enabled:
            trading_pair_cards[inst_id].update_price(price)

        # 将价格数据传递给 AlertProcessor
        if alert_processor_instance and trading_pair_cards[inst_id].is_enabled:  # 仅当监控启用时处理预警
            # AlertProcessor 的 process_price_data 目前是同步的，如果未来改为异步，这里需要 await
            alert_processor_instance.process_price_data(pair_id, price)
            # logger.debug(f"价格数据已发送至AlertProcessor: pair_id={pair_id}, inst_id={inst_id}, price={price}")
...
async def _handle_save_alert_rule(rule_data: AlertRule, card_inst_id_to_refresh: str):
    ...
    is_new_rule = not rule_data.id # 检查是否有ID来判断是新建还是更新

    if not is_new_rule and rule_data.id is not None: # 更新现有规则
        # 从待更新数据中排除 ID, pair_id (通常不应更改), 和运行时状态 last_triggered_timestamp
        update_payload = rule_data.model_dump(exclude={'id', 'pair_id', 'last_triggered_timestamp'})
        ...
        if new_rule_id:
            ...
            rule_data.id = new_rule_id # 确保 rule_data 对象也拥有ID，以便后续使用
...
async def _open_rule_editor(pair_id: int, inst_id: str, rule_to_edit: Optional[AlertRule] = None):
    """打开规则编辑器对话框"""
    # 修改 on_save 的处理方式
    # RuleEditorForm 的 on_save 回调期望一个 Callable[[AlertRule], None]
    # _handle_save_alert_rule 是 async，NiceGUI 的事件系统可以处理 async 回调
    async def save_wrapper(rule: AlertRule): # 创建一个 wrapper 来匹配参数签名并调用
        await _handle_save_alert_rule(rule, inst_id)

    editor = RuleEditorForm(
        trading_pair_id=pair_id,
        inst_id=inst_id,
        on_save=save_wrapper, # 直接传递 async 函数或包装器
...
async def _handle_delete_alert_rule(rule_id: int, pair_id: int, card_inst_id_to_refresh: str):
    ...
    rule_to_delete = db_manager.get_alert_rule_by_id(rule_id)  # 获取规则名称用于提示
            ...
            async def do_delete():
                ...
                if db_manager.delete_alert_rule(rule_id):
                    ...
                    if alert_processor_instance:
                        # 从 AlertProcessor 缓存中移除或重新加载该pair的规则
                        # 一个简单的方法是重新加载pair的规则，或者AlertProcessor提供一个remove_rule方法
                        # 为了简单，我们先重新加载整个pair的规则
...
async def _handle_toggle_alert_rule_enabled(rule_id: int, new_status: bool, pair_id: int, card_inst_id_to_refresh: str):
    """处理切换预警规则的启用状态"""
    global alert_processor_instance
    if db_manager.update_alert_rule(rule_id, {"is_enabled": new_status}):
        ...
        await _refresh_card_rules(card_inst_id_to_refresh)  # 刷新UI
    else:
        ui.notify("更新规则状态失败。", type='error')
        await _refresh_card_rules(card_inst_id_to_refresh)  # 即使失败也刷新以回滚UI开关（如果需要）
    ...
    with confirm_dialog, ui.card():
        ...
        with ui.row().classes('justify-end w-full'):
            ...
            async def do_delete():
                ...
                if db_manager.delete_trading_pair(pair_id):  # 级联删除应处理其下的alert_rules
                    if was_enabled:
                        if pcm_instance:
                            await pcm_instance.unsubscribe_mark_price(inst_id)
                            pcm_instance.unregister_price_update_callback(inst_id)
...
async def setup_page_content():
    ...
    with ui.column().classes('items-center w-full q-pa-md'):
        ...
        if current_cards_container:
            ...
            cards_container = ui.grid(columns=3).classes('gap-4 w-full max-w-5xl')  # 稍微加宽
            current_cards_container = cards_container

        if pcm_instance:  # 清理旧订阅和回调
        ...
        # alert_processor 也可能需要清理，但它目前是基于 pair_id 的，会在加载时重建
        if alert_processor_instance:
            for pair_id_old in list(alert_processor_instance._instId_map.keys()):  # 假设有此映射
        ...
        with current_cards_container:
            for pair_model in db_pairs:
                ...
                card = TradingPairCard(
                    ...
                    # 传递预警规则相关的回调，并绑定 inst_id 以便刷新正确的卡片
                    on_add_rule=lambda p_id=pair_model.id, i_id=pair_model.instId: _open_rule_editor(p_id, i_id),
                    on_edit_rule=lambda rule_model, i_id=pair_model.instId: _open_rule_editor(rule_model.pair_id, i_id,
                                                                                              rule_model),
                    on_delete_rule=lambda r_id, p_id, i_id=pair_model.instId: _handle_delete_alert_rule(r_id, p_id,
                                                                                                        i_id),
                    on_toggle_rule_enabled=lambda r_id, status, p_id,
                                                  i_id=pair_model.instId: _handle_toggle_alert_rule_enabled(r_id,
                                                                                                            status,
                                                                                                            p_id, i_id),
                    initial_rules=rules_for_pair  # 传递初始规则
                )
                trading_pair_cards[pair_model.instId] = card

                if pair_model.is_enabled:  # 如果交易对启用
                    if pcm_instance:
                    ...
                    if alert_processor_instance:  # 加载规则到AlertProcessor
                ...
                if pair_db_id and current_cards_container:
                    ...
                    with current_cards_container:
                        card = TradingPairCard(
                            ...
                            on_edit_rule=lambda rule_model, i_id=inst_id: _open_rule_editor(rule_model.pair_id, i_id,
                                                                                            rule_model),
                            on_delete_rule=lambda r_id, p_id, i_id=inst_id: _handle_delete_alert_rule(r_id, p_id, i_id),
                            on_toggle_rule_enabled=lambda r_id, status, p_id,
                                                          i_id=inst_id: _handle_toggle_alert_rule_enabled(r_id, status,
                                                                                                          p_id, i_id),
                            initial_rules=rules_for_new_pair,  # 传递初始规则（应为空）
                            initial_price="待订阅..."
                        )
                    ...
                    if pcm_instance:  # 订阅新添加的
                        await pcm_instance.subscribe_mark_price(inst_id, resubscribe_check=False)
                        pcm_instance.register_price_update_callback(
                            inst_id, price_update_handler_factory(pair_db_id, inst_id)  # 传递 pair_db_id
                        )
                    if alert_processor_instance:  # 为新交易对加载规则（即使为空）
...
async def on_app_startup():
    global pcm_instance, alert_processor_instance  # 添加 alert_processor_instance
    logger.info("应用启动...")
...
async def on_app_shutdown():
    ...
    logger.info("应用关闭...")
    if pcm_instance:
        for inst_id in list(trading_pair_cards.keys()):  # 使用 trading_pair_cards 作为参考
            card = trading_pair_cards.get(inst_id)
            if card and card.pair_id and alert_processor_instance:  # 清理AlertProcessor
...
if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s [%(levelname)s] %(name)s [%(name)s:%(lineno)d] %(message)s',  # 更详细的日志格式
                        datefmt='%H:%M:%S')
